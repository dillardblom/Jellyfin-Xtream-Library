...
Most endpoints require admin authorization (`RequiresElevation` policy). The Live TV playlist/EPG endpoints (`LiveTv.m3u`, `Epg.xml`, `Catchup.m3u`) and `ChannelLogo/{streamId}` are anonymous so external players and the server's own image fetcher can reach them; Jellyfin's M3U tuner and XMLTV fetcher send no Jellyfin credentials, so requiring auth would break them. **The consequence is that the playlist endpoints hand out the Xtream username and password without a login**, because `LiveTvService.BuildStreamUrl` interpolates them into every stream line and the Xtream protocol has no token form. Jellyfin treated the equivalent gap in its own `LiveTvController` as a bug ([jellyfin#5415](https://github.com/jellyfin/jellyfin/issues/5415), resolved in 10.8.9); see the Security Note in the README for what mitigates it here. `ChannelLogo` resolves a local-path channel override logo (via `ChannelLogoResolver`) and serves the file; the request carries only a stream ID, so no filesystem path is taken from the caller.
